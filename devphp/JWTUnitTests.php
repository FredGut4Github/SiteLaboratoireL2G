...
class JWTUnitTest
{
	public static function assertEquals($msg,$res)
	{
		if ($msg === $res)
		{
			echo "<td style='color:green; width:100px; text-align:right'>OK</td>";
		}
		else
		{
			echo "<td style='color:red; width:100px; text-align:right'>Failed</td>";
		}
	}
    ...
    public static function testEncodeDecode()
    {
        ...
        $result = JWT::decode($msg, 'my_key', array('HS256'));
		echo "<td>function testEncodeDecode(): 'abc' --> ".$msg." --> '".$result."'</td>";
		JWTUnitTest::assertEquals($result,'abc');
    	echo "</tr>";
    }
    public static function testDecodeFromPython()
    {
    	echo "<tr>";
		...
		$result = JWT::decode($msg, 'my_key', array('HS256'));
		echo "<td>function testDecodeFromPython()</td>";
        JWTUnitTest::assertEquals($result,'*:http://application/clicky?blah=1.23&f.oo=456 AC000 123');
    	echo "</tr>";
    }
    public static function testUrlSafeCharacters()
    {
    	echo "<tr>";
        $encoded = JWT::encode('f?', 'a');
		$result = JWT::decode($encoded, 'a', array('HS256'));
		echo "<td>function testUrlSafeCharacters(): 'f?' --> ".$encoded." --> '".$result."</td>";
        JWTUnitTest::assertEquals('f?', $result);
    	echo "</tr>";
    }
 ...
 */
    ...
    public static function testValidToken()
    {
    	echo "<tr>";
        $payload = array(
            "message" => "abc",
		...
		echo "<td>function testValidToken(): ".serialize($decoded)." / \$decoded->message === 'abc'</td>";
        JWTUnitTest::assertEquals($decoded->message, 'abc');
    	echo "</tr>";
    }
